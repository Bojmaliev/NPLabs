...
final class IntegerArray {
    ...
    IntegerArray getSorted() {
        return new IntegerArray(Arrays.stream(a).sorted().toArray());
    }
    IntegerArray concat(IntegerArray ia) {
        int nova[] = Arrays.copyOf(a, length()+ia.length());
        for(int i=length(); i<length()+ia.length(); i++)nova[i] = ia.getElementAt(i-length());
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder();
        sb.append("[");
            for(int i=0; i<length()-1; i++)sb.append(getElementAt(i)).append(", ");
            sb.append(getElementAt(length()-1));
        sb.append("]");
        return sb.toString();
    }
