...
class Timestamp<T> implements Comparable<Timestamp<?>> {
    ...
    public boolean equals(Object obj) {
        return time.equals(((Timestamp)obj).time);
    }
...
class Scheduler<T>{
    ...
    Timestamp<T> next(){
        return timeStamps.stream().filter(a-> a.time.isAfter(LocalDateTime.now())).sorted().findFirst().get();
    }
    Timestamp<T> last(){
        return timeStamps.stream().filter(a->a.time.isBefore(LocalDateTime.now())).sorted((o1, o2) -> -o1.compareTo(o2)).findFirst().get();
    }
    List<Timestamp<T>> getAll(LocalDateTime begin, LocalDateTime end){
        return timeStamps.stream().filter(a->a.time.isAfter(begin) && a.time.isBefore(end)).collect(Collectors.toList());
    }
