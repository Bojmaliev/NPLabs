...
class Contact{
    ...
    Contact(String name, String ...phonenumber) throws Exception{
        this.name = name;
    ...
    void addNumber(String phoneNumber) throws Exception{
...
class PhoneBook{
    ...
    void addContact(Contact contact) throws Exception{
        ...
        if(contactExists(contact.name)) throw new InvalidNameException(contact.name);
    ...
    Contact getContactForName(String name) throws Exception{
        if(contactExists(name)){
            return Arrays.stream(contacts).filter(a-> a.name.equals(name)).findFirst().get();
        }
        return null;
